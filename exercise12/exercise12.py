...
def prepare_xy(df: pd.DataFrame):
    ...
    if "SERVER" in X.columns:
        X["SERVER_VENDOR"] = bucket_server_vendor(X["SERVER"])
        X = X.drop(columns=["SERVER"])  # replace raw with bucket
...
if __name__ == "__main__":
    ...
    unique_servers = set(df["SERVER"].dropna().astype(str).str.lower())
    print(len(unique_servers))
    for s in sorted(unique_servers):
        print(s)
